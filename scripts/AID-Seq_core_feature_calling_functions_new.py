...
def find_initial_read_pileups(bam_file,chromname,banned_chroms=["alt", "random", "Un"],
                              depth_thresh=5):
    peak_locations = []
    # if len(initial_peaks) > 0:
    #     for initial_peak in initial_peaks:
    #         chrom, start, end = parse_coordinate(initial_peak)
    #         if chrom == chromname:
    #             peak_locations.append(initial_peak)
    # else:
    """
    get peak file(split by chrom)
    """
    bam=pysam.AlignmentFile(bam_file, "rb")
    pileup = bam.pileup(contig=chromname) #pileup by chrom
    in_peak = False
...
"""def call_aid_seq_features(initial_peaks, bam_file,
...
"""def call_aid_seq_features(initial_peaks_file, bam_file,
                           ...
                           banned_chroms=["alt", "random", "Un", "chrM"]):"""
## multithreading (hong）
def work(genome,initial_peaks,banned_chroms,bam_file,layout,min_dup,
         min_read_length,flank_from_feature,sample_name):
    features = []
    ...
    #add write into .fa file(by chrom)
    fa_file_name=sample_name+"_"+chromname
    fasta_file = '{0}_candidate.fa.tmp'.format(fa_file_name)
    fasta_from_features(fasta_file, features, pyfaidx_fasta_obj)
    return features

def call_aid_seq_features(layout,
                           ...
                           required_cliff_ratio=0.9,
                           banned_chroms=["alt", "random", "Un", "chrM"]):
    chr=["chr1","chr2","chr3","chr4","chr5","chr6","chr7","chr8","chr9","chr10","chr11","chr12","chr13","chr14","chr15",
         "chr16","chr17","chr18","chr19","chr20","chr21","chr22","chrX","chrY"]
    sample_name = os.path.splitext(bam_file)[0]
    depth_thresh=5
    result_peak=[]
    peakl_chrom=[]
    #peaks file is not none;
    if not initial_peaks_file is None:
        for chrname in range(len(chr)):
            peakl_chrom.append([])
        with open(initial_peaks_file, 'r') as file:
            for initial_peaks in file:
                chrom, start, end = parse_coordinate(initial_peaks)
                for chrname in range(len(chr)):
                    if chrom == chr[chrname]:
                        peakl_chrom[chrname].append(initial_peaks)
    else:
        #when peaks file is none;multiprocess get peak
        pool_peak = Pool(24)
        for i in range(len(chr)):
            """
            chromname,genome,initial_peaks_file,banned_chroms,bam,layout,min_dup,
             min_read_length,flank_from_feature,features_q,peakl_q
            """
            result_peak.append(pool_peak.apply_async(find_initial_read_pileups,(bam_file,chr[i],banned_chroms,depth_thresh,)))
        pool_peak.close()
        pool_peak.join()
        peakl=[]
        for j in result_peak:
            peakl_chrom.append(j.get())
            peakl.extend(j.get())
        peak_file = '{0}.peak'.format(sample_name)
        with open(peak_file, "w+") as f:
            for peak in peakl:
                f.write(peak + "\n")
    #multiprocess get candidate.fa
    pool_fa = Pool(24)
    result_features=[]
    for x in range(len(peakl_chrom)):
        result_features.append(pool_fa.apply_async(work,(genome,peakl_chrom[x],
                                             banned_chroms,bam_file,layout,min_dup, min_read_length,
                                             flank_from_feature,sample_name,)))
    pool_fa.close()
    pool_fa.join()
    features = []
    for y in result_features:
        features.extend(y.get())

    tmp_list=[]
    for c in chr:
        filename=sample_name+"_"+c+"_candidate.fa.tmp"
        tmp_list.append(filename)
    # pyfaidx_fasta_obj=Fasta(genome)
    # fasta_file = '{0}_candidate.fa'.format(sample_name)
    # fasta_from_features(fasta_file, features, pyfaidx_fasta_obj)
    return features,tmp_list
...
def call_aid_seq_features_form_file(candidate_fasta_file, genome):
    features = []
...
def parse_args():
    ...
    parser.add_argument('--flank_from_feature', '-f', help='Sets the sequence around the AID-Seq feature to return in the fasta file output by the function.', default=20, type=int)
...
def main():
    ...
    if not args.matched_file is None:
        ...
        if not args.candidate_fasta_file is None:
            features = call_aid_seq_features_form_file(args.candidate_fasta_file, args.genome)
        else:
            features,tmp_list = call_aid_seq_features(args.layout,
                                              ...
                                              args.banned_chroms)
            #add
            # ##in the end (cat all temp .fa file)
            sample_name = os.path.splitext(args.bam_file)[0]
            fasta_file = '{0}_candidate.fa'.format(sample_name)
            f=open(fasta_file,'w+')
            for fa_tmp in tmp_list:
                #print("cat",fa_tmp,"now ....")
                for line in open(fa_tmp):
                    f.writelines(line)
            f.close()
            #after cat tmp file ;remove all tmp file
            for tmp in tmp_list:
                #print("delete tmp file :",tmp,"now ....")
                os.remove(tmp)
            #end
        outfile_matched = output_alignments(args.out_name, args.target_sequence.upper(), features, args.mismatch_threshold)
...
if __name__ == '__main__':
    ...
    main()
    #T_end = time.clock()
    #print("pipeline time : %s "%(T_end-T_star))
